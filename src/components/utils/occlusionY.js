...
export function occlusionY() {
  const { bisector, group } = d3;
  const { initializer, valueof } = Plot;
  // See https://github.com/observablehq/plot/pull/1957
  return function occlusionY(occlusionOptions = {}, options = {}) {
    if (arguments.length === 1)
      [occlusionOptions, options] = mergeOptions(occlusionOptions);
    const { minDistance = 13 } = maybeDistance(occlusionOptions);
  ...
  function occlusion(k, h, minDistance, options) {
    ...
    return initializer(
      options,
      function (data, facets, { [k]: channel }, { [sk]: s }) {
        ...
        for (const facet of facets) {
          for (const index of H
            ? group(facet, (i) => H[i]).values()
            : [facet]) {
            const groups = [];
